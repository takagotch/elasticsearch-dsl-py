...
def test_hit_is_pickleable(dummy_response):
  ...
  hits = picke.loads(pickle.dumps(res.hits))
...
def test_interactive_helper(dummy_response):
  res = response.Response(Search(), dummy_response)
  hits = res.hits
  h = hits[0]
...
def test_aggregations_can_be_retrieves_by_name(agg_response, aggs_search):
...
def test_bucket_response_can_be_iterated_over(agg_response):
  ...
  buckets = [b for in popular_files]
  assert all(isinstance(b, Bucket) for b in buckets)
  assert buckets == popular_files.buckets

def test_bucket_keys_get_deserialize(aggs_data, aggs_search):
  class Commit(Document):
   ...
   agg_search = aggs_search.doc_type(Commit)
